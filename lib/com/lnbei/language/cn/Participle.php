<?php
namespace com\lnbei\language\cn;
/**
 * 中文分词
...
class Participle
{
    ...
    function __construct($loaddic=true) {
        if($loaddic) {
            for($i=0;$i<strlen($this->cn_one_name);$i++){
                if($i+1 < strlen($this->cn_one_name)){
    ...
    function get_source($str) {
        if('utf-8' == 'utf-8') $str = iconv('utf-8','gbk',$str);
    ...
    function split_result($str='',$try_num_name=true,$try_diff=true) {
        ...
        if('utf-8'=='utf-8') $okstr = iconv('gbk','utf-8',$this->result_string);
    ...
    function get_keyword($str,$ilen=-1) {
        ...
        if('utf-8'=='utf-8') $okstr = iconv('gbk','utf-8',$okstr);
        return trim($okstr);
    }
    function get_rank($w){
     ...
     */
    //$f = new Participle();
    // $segment_q = $f->get_keyword($f->split_result("河南省你们好"));
    // echo "dd";
