...
class GoodCatsModel extends Model
{
    ...
    {
        $result = $this->where(['dataFlag'=>1, 'parentId'=>$parentid, 'shopId'=>1])->select();
        $goods_result=array();
    ...
    /**
     * [getallCats 该门店获取所有分类]
     * @return [type] [description]
     * @author
     */
    public function getallCats($shopId){
        $result = $this -> where('catId','<>', 0)->update(["shopId" => $shopId]);
        return $result;
    }
}