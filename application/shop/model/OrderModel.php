...
class OrderModel extends Model
{
    ...
    public function buildOrder($data){
        $address = Db::name("user_address")->cache(true,10)->where(["userId"=>$data['userID'],"isDefault"=>1])->find();
        if($address == null){
            return null;
        }
        $order = array();
        ...
        $action['log_time'] = date('Y-m-d H:i:s');
        
        $goodresult = Db::name("goods")->cache(true,10)->where("goodsId",$data['goodsId'])->find();//查询商品
        $goods = array();
    ...
    public function getOrdersByWhere($map, $Nowpage, $limits)
    {
        ...
        return $this
        ->cache(true,10)->where($map)->page($Nowpage, $limits)->order('xx_order.order_id desc')->select();
    }
    ...
    public function buildOrderFromCard($data,$datashop){
            ...
            $address = Db::name("user_address")->cache(true,10)->where("userId",$data['userID'])->find();
            if($address == null){
                return null;
            }
            $order = array();
            ...
            $order['twon'] = $addressint[2];
                ...
                try{
                    ...
                    foreach ($shopid as $key =>$value){
                        if($value == ""){break;}
                        $goodresult = Db::name("cart")->cache(true,10)->where("id",$value)->find();//查询商品
                        ...
                        Db::name("cart")->cache(true)->where("id",$value)->delete();
                    }
    ...
    public function getOneOrder($id){
        return $this ->cache(true,10)->where("order_id",$id)->find();
    }
    //更新数据库
