...
use app\mall\model\OrderActionModel;
...
{    /**
    ...
    public function index(){
        ...
        $limits = 10;// 获取总条数
        $count = Db::name('order')->cache(true,60)->where($map)->count();//计算总页面
    ...
    //使用积分
    public function ajaxUseIntegral(){
        $order_id = input("post.orderid");//订单编号
        $integral = input("post.integral");//积分
        $user = new UserModel();
        $userID = Session::get("userId");
        $userInfo = $user->getOneUser($userID);
        if($userInfo["userScore"] < $integral ||$integral <=0){
            
            return json(array("code"=>0,"msg"=>"积分输入无效"));
        }
        $order = new OrderModel();
        $orderInfo = $order->getOneOrder($order_id);
        if($orderInfo == null){
            return json(array("code"=>0,"msg"=>"无此订单"));
        }
        $goodsPrice = $orderInfo["order_amount"];
        $exchange = $integral * 0.1;
        if($exchange > $goodsPrice - 20){
            
            return json(array("code"=>0,"msg"=>"不能这样"));
        }
        $goodsPrice -= $exchange;
        $userintegral = $userInfo["userScore"]-$integral;
        $order->startTrans();
        try{
           $order ->updateOrder(["integral"=>$integral,"integral_money"=>$exchange,"order_amount"=>$goodsPrice],["order_id"=>$order_id]) ;
           $user->save(["userScore"=>$userintegral],["userId"=>$userID]) ;
           $order->commit();
        }catch(\Exception $e){
                    $this->rollback();
                   return json(array("code"=>0,"msg"=>"出错"));
        }
        return json(array("code"=>1,"msg"=>"成功","data"=>$goodsPrice,"Userintegral"=>$userintegral));
    }
}
